...
namespace InfoHub.Helpers
{
    ...
    {
        private LicenseInformation m_License;
        private ListingInformation m_Listing;
        private ProductListing m_Product;
        ...
        public InAppPurchaseHelper(string featureKey, bool simulate = true)
        {
            ...
            Simulate = simulate;
            CanPurchase = false;
            System.Diagnostics.Debug.WriteLine(string.Format("InAppPurchaseHelper({0}, {1})", featureKey, simulate));
        }
        ...
        public bool IsPurchased
        {
            get
            {
                try { return m_License.ProductLicenses[this.Key].IsActive; }
                catch (Exception e)
                {
                    System.Diagnostics.Debug.WriteLine(string.Format("IsPurchased [{0}, {1}, {2}]", this.Key, this.Simulate, e.Message));
                    return false;
                }
            }
        }
        ...
        public DateTime Expires
        {
            get
            {
                try { return m_License.ExpirationDate.DateTime; }
                catch (Exception e)
                {
                    System.Diagnostics.Debug.WriteLine(string.Format("Expires [{0}, {1}, {2}]", this.Key, this.Simulate, e.Message));
        ...
        public string FormattedPrice
        {
            get
            {
                try { return m_Product.FormattedPrice; }
                catch (Exception e)
                {
                    System.Diagnostics.Debug.WriteLine(string.Format("FormattedPrice [{0}, {1}, {2}]", this.Key, this.Simulate, e.Message));
                    return string.Empty;
                }
            }
        }
        ...
        public static bool HasInternetAccess
        {
            get
            {
                try
                {
                    ...
                    if (_Profile == null)
                        return false;
                    return _Profile.GetNetworkConnectivityLevel() == NetworkConnectivityLevel.InternetAccess;
                }
                catch (Exception e)
                {
                    System.Diagnostics.Debug.WriteLine(string.Format("HasInternetAccess [{0}]", e.Message));
                    return false;
                }
            }
        }
        ...
        public async Task<bool> Purchase()
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine(string.Format("Purchase [{0}, {1}, {2}]", this.Key, this.Simulate, e.Message));
                Debugger.Break();
            }
